<h1 align="center">Arhum Fareed</h1>

<p align="center">
  <strong>Full Stack Developer</strong> &nbsp;/&nbsp; <em>AI Builder</em><br>
  <sub>Islamabad, Pakistan &nbsp;·&nbsp; Remote / Onsite &nbsp;·&nbsp; Open to opportunities</sub>
</p>

<p align="center">
  <a href="https://arhum1391.github.io/portfolio/"><img src="https://img.shields.io/badge/portfolio-live-FF4A1C?style=flat-square&labelColor=0A0A0C" alt="Portfolio"></a>
</p>

---

### `(01) / About`

I ship things that live in production.

Full Stack Developer at **Ideofuzion**, BS Artificial Intelligence student at **COMSATS** (7th semester), and CS Department Lead at **ResearchCircle**. Somewhere in there I sleep.

The through-line is integration: getting independent services to behave as one coherent system, and designing sensible failure paths for the moments a request never comes back. Mostly I spend my days making services talk to each other that were never meant to meet.

---

### `(02) / Selected work`

Four platforms taken from an empty repository to production traffic.

| | Project | Role | What it actually does |
|---|---|---|---|
| **01** | **Influencer Analyst Platform** | Lead Full Stack Developer | Subscription and bootcamp platform built from scratch as the sole developer. Stripe for payments, Calendly for 1-on-1 scheduling, deployed and maintained on production servers. I also managed the Figma designers, QA and PMs, which is its own kind of async work. |
| **02** | **NFT Marketplace Platform** | Product & Engineering | End-to-end architecture for blockchain-based digital asset trading. Product planning through feature development to deployment: I owned the shape of the system as well as the code. |
| **03** | **AI Hiring & ATS Platform** | AI & Automation Engineer | A hiring pipeline that runs itself. n8n pulls CVs straight out of the inbox, shortlists against the role, and books interviews without a recruiter touching a calendar. Vexa transcribes the call live, then AI grades it on behavioural and technical axes and hands back one confidence score. |
| **04** | **Saudi Fintech Loan Management** | Full Stack Developer | Peer-to-peer lending for the Saudi market. Borrowers clear debt, lenders manage investments, banking APIs move real money in real time. The kind of project where an off-by-one is not a rounding error. |

---

### `(03) / The stack`

Everything here has been used in something that shipped.

**Languages**

![Python](https://img.shields.io/badge/Python-0A0A0C?style=flat-square&logo=python&logoColor=EFEAE1)
![JavaScript](https://img.shields.io/badge/JavaScript-0A0A0C?style=flat-square&logo=javascript&logoColor=EFEAE1)
![Java](https://img.shields.io/badge/Java-0A0A0C?style=flat-square&logo=openjdk&logoColor=EFEAE1)
![C++](https://img.shields.io/badge/C++-0A0A0C?style=flat-square&logo=cplusplus&logoColor=EFEAE1)
![SQL](https://img.shields.io/badge/SQL-0A0A0C?style=flat-square&logo=mysql&logoColor=EFEAE1)

**Web & Frameworks**

![React](https://img.shields.io/badge/React.js-0A0A0C?style=flat-square&logo=react&logoColor=EFEAE1)
![Node.js](https://img.shields.io/badge/Node.js-0A0A0C?style=flat-square&logo=nodedotjs&logoColor=EFEAE1)
![Tailwind](https://img.shields.io/badge/Tailwind_CSS-0A0A0C?style=flat-square&logo=tailwindcss&logoColor=EFEAE1)
![HTML5](https://img.shields.io/badge/HTML5-0A0A0C?style=flat-square&logo=html5&logoColor=EFEAE1)
![CSS3](https://img.shields.io/badge/CSS3-0A0A0C?style=flat-square&logo=css3&logoColor=EFEAE1)

**AI & Automation**

![LLM Integration](https://img.shields.io/badge/LLM_Integration-0A0A0C?style=flat-square&logo=openai&logoColor=EFEAE1)
![n8n](https://img.shields.io/badge/n8n-0A0A0C?style=flat-square&logo=n8n&logoColor=EFEAE1)
![Vexa](https://img.shields.io/badge/Vexa-0A0A0C?style=flat-square&logoColor=EFEAE1)
![ATS Models](https://img.shields.io/badge/Automated_ATS_Models-0A0A0C?style=flat-square&logoColor=EFEAE1)
![Matplotlib](https://img.shields.io/badge/Matplotlib-0A0A0C?style=flat-square&logo=python&logoColor=EFEAE1)

**Services & Tooling**

![Stripe](https://img.shields.io/badge/Stripe-0A0A0C?style=flat-square&logo=stripe&logoColor=EFEAE1)
![Calendly](https://img.shields.io/badge/Calendly-0A0A0C?style=flat-square&logo=calendly&logoColor=EFEAE1)
![Banking APIs](https://img.shields.io/badge/Banking_APIs-0A0A0C?style=flat-square&logoColor=EFEAE1)
![Git](https://img.shields.io/badge/Git-0A0A0C?style=flat-square&logo=git&logoColor=EFEAE1)
![Postman](https://img.shields.io/badge/Postman-0A0A0C?style=flat-square&logo=postman&logoColor=EFEAE1)
![Figma](https://img.shields.io/badge/Figma-0A0A0C?style=flat-square&logo=figma&logoColor=EFEAE1)
![Vite](https://img.shields.io/badge/Vite-0A0A0C?style=flat-square&logo=vite&logoColor=EFEAE1)

---

### `(04) / The path`

```
2026 →  CS Department Lead, ResearchCircle
        Circle Leads, project timelines, and turning student research into
        professional-grade output.

2025 →  Junior Full Stack Developer, Ideofuzion
        Responsive apps on modern stacks. Features shipped, critical bugs killed,
        agile cycles survived.

2025 ·  AI Intern, Ideofuzion
        AI wired into ATS models, models hosted on cloud servers, n8n workflows
        built and debugged.

2023 ·  BS Artificial Intelligence, COMSATS University
        Machine Learning, Neural Networks, Software Engineering. Currently 7th semester.
```

---

### `(05) / What I am good at`

- **Full stack delivery.** Responsive applications built and maintained across front end and back end.
- **Payments and subscriptions.** Stripe billing and Calendly scheduling, wired in end to end rather than bolted on.
- **Fintech.** Banking API integrations carrying real transactions and real verification, with the failure paths designed on purpose.
- **AI and workflow automation.** LLMs and n8n put to work on actual business processes, not demos.
- **Cloud hosting and deployment.** Getting it to production, and keeping it there.
- **Cross-functional leadership.** Designers, QA and PMs pointed at the same milestone.

---

### `(06) / Elsewhere`

<p>
  <a href="https://arhum1391.github.io/portfolio/"><img src="https://img.shields.io/badge/Portfolio-FF4A1C?style=for-the-badge&logoColor=0A0A0C" alt="Portfolio"></a>
  <a href="https://www.linkedin.com/in/arhum-fareed-b67353376/"><img src="https://img.shields.io/badge/LinkedIn-0A0A0C?style=for-the-badge&logo=linkedin&logoColor=EFEAE1" alt="LinkedIn"></a>
</p>

Email, hover the block and hit copy:

```
arhumfareed01@gmail.com
```

---

<p align="center"><sub>Built from scratch. Still building.</sub></p>
